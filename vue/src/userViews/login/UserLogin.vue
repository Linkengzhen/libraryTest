...
<div class="wrapper">
  <div style="margin: 200px auto; background-color: #fff; width: 350px; height: 350px; padding: 20px; border-radius: 10px;position: relative">
    ...
    <div style="margin: 10px 0; text-align: center; font-size:16px"><b>读者登录</b></div>
    <form>
    <el-form :model="user" :rules="rules" ref="loginForm">
      <el-form-item prop="username" >
      <el-input size="medium" style="margin: 10px 0" prefix-icon="el-icon-user" placeholder="请输入账号" v-model="user.username"></el-input>
      </el-form-item >
      <el-form-item prop="password">
      <el-input size="medium" style="margin: 10px 0" prefix-icon="el-icon-lock" type="password" show-password placeholder="请输入密码" v-model="user.password"></el-input>
      </el-form-item>
      <el-form-item style="margin: 10px 0; text-align: center">
        <el-radio v-model="radio" label="1" @input="$router.push('/adminLogin')">管理员</el-radio>
        <el-radio v-model="radio" label="2" >读者</el-radio>
      </el-form-item>
      <el-form-item style="margin: 20px 0; text-align: right">
        <el-button type="primary" size="small" autocomplete="off" @click="login">登录</el-button>
        <el-button type="warning" size="small" autocomplete="off" @click="$router.push('/userRegister')">注册</el-button>
      </el-form-item>
    </el-form>
    </form>
  </div>
  ...
  methods: {
    login(){
      this.$refs["loginForm"].validate((valid) => {
        if(valid){
          request.post('/login/userLogin',this.user).then(res => {
            if(res.code==='200'){
